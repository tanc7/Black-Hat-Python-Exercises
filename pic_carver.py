...
def http_assembler(pcap_file):
    ...
    for session in sessions:
        ...
        if image is not None and image_type is not None:
            # store the image
                file_name = "%s-pic_carber_%d.%s" % (pcap_file,carved_images,image_type)
...
def extract_image(headers,http_payload):
    ...
    try:
        ...
        return None,None
    return iamge,image_type

def face_detect(path,file_name):
    img = cv2.imread(path)
    cascade = cv2.CascadeClassifier("harrcascade_frontalface_alt.xml")
    ...
    # highlight the faces in the image_type
